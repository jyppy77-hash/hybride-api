...
Architecture (mirrors decay_state V94 pattern):
    ...
    - Idempotent via UNIQUE KEY (game, draw_date_target, number_value, number_type)
        ...
        for ntype, values in (selected_numbers or {}).items():
            ...
            for n in values:
                ...
                await cursor.execute(
                    ...
                    "(game, number_value, number_type, draw_date_target) "
                    "VALUES (%s, %s, %s, %s)",
...
async def get_persistent_brake_map(
    ...
    try:
        ...
        await cursor.execute(
            ...
            "WHERE game = %s AND number_type = %s AND draw_date_target < %s "
        ...
        await cursor.execute(
            ...
            f"WHERE game = %s AND number_type = %s "
...
async def cleanup_old_selections(
    ...
    try:
        ...
        await cursor.execute(
            "SELECT DISTINCT draw_date_target FROM hybride_selection_history "
            "WHERE game = %s ORDER BY draw_date_target DESC LIMIT %s",
...
async def is_first_generation_of_target_draw(
    ...
    try:
        ...
        await cursor.execute(
            ...
            "WHERE game = %s AND draw_date_target = %s LIMIT 1",
