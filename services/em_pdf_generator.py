"""
PDF Generator — EuroMillions META ANALYSE
Genere un rapport PDF A4 avec DEUX graphiques (boules + etoiles).
Equivalent EM de services/pdf_generator.py (ZERO modification au fichier Loto).
"""

import os
import io
import tempfile
import logging

from config.version import APP_VERSION

logger = logging.getLogger(__name__)


PDF_LABELS = {
    "fr": {
        "title": "Rapport META DONN\u00c9E EM - 75 Grilles",
        "subtitle": "Analyse HYBRIDE EuroMillions",
        "top5_bar": "Top 5 Boules - Fr\u00e9quences",
        "top5_pie": "R\u00e9partition Top 5 Boules",
        "top3_bar": "Top 3 \u00c9toiles - Fr\u00e9quences",
        "top3_pie": "R\u00e9partition Top 3 \u00c9toiles",
        "frequency": "Fr\u00e9quence",
        "balls": "Boules",
        "stars": "\u00c9toiles",
        "analysis": "Analyse :",
        "no_analysis": "Aucune analyse disponible.",
        "info": "Informations :",
        "window_label": "Fen\u00eatre analys\u00e9e",
        "engine_label": "Moteur",
        "game_label": "Jeu : EuroMillions (5 boules + 2 \u00e9toiles)",
        "sponsor_title": "Analyse offerte par un Partenaire Officiel",
        "sponsor_contact": "Partenariat & sponsoring :",
        "signature": "Analyse g\u00e9n\u00e9r\u00e9e par HYBRIDE",
        "engine_desc": "Moteur statistique p\u00e9dagogique - Module EuroMillions",
        "generated_by": "G\u00e9n\u00e9r\u00e9 par LotoIA - Module META DONN\u00c9E EM",
        "disclaimer1": "Analyse statistique p\u00e9dagogique.",
        "disclaimer2": "Aucun r\u00e9sultat n\u2019est garanti.",
        "version": "LotoIA \u2014 Rapport META DONN\u00c9E EM",
        "ai_note": "Ce rapport est enti\u00e8rement g\u00e9n\u00e9r\u00e9 par intelligence artificielle en collaboration avec le moteur HYBRIDE.",
        "dev_note": "Graphiques et visuels en cours de d\u00e9veloppement.",
        "heatmap_title_balls": "Fr\u00e9quences compl\u00e8tes - Boules (1-50)",
        "heatmap_title_stars": "Fr\u00e9quences compl\u00e8tes - \u00c9toiles (1-12)",
        "heatmap_legend_cold": "Froid (rare)",
        "heatmap_legend_hot": "Chaud (fr\u00e9quent)",
    },
    "en": {
        "title": "META DATA EM Report - 75 Grids",
        "subtitle": "HYBRIDE EuroMillions Analysis",
        "top5_bar": "Top 5 Balls - Frequencies",
        "top5_pie": "Top 5 Balls Distribution",
        "top3_bar": "Top 3 Stars - Frequencies",
        "top3_pie": "Top 3 Stars Distribution",
        "frequency": "Frequency",
        "balls": "Balls",
        "stars": "Stars",
        "analysis": "Analysis:",
        "no_analysis": "No analysis available.",
        "info": "Information:",
        "window_label": "Window analysed",
        "engine_label": "Engine",
        "game_label": "Game: EuroMillions (5 balls + 2 stars)",
        "sponsor_title": "Analysis sponsored by an Official Partner",
        "sponsor_contact": "Partnerships & sponsoring:",
        "signature": "Analysis generated by HYBRIDE",
        "engine_desc": "Educational statistical engine - EuroMillions Module",
        "generated_by": "Generated by LotoIA - META DATA EM Module",
        "disclaimer1": "Educational statistical analysis.",
        "disclaimer2": "No results are guaranteed.",
        "version": "LotoIA \u2014 META DATA EM Report",
        "ai_note": "This report is entirely generated by artificial intelligence in collaboration with the HYBRIDE engine.",
        "dev_note": "Charts and visuals under development.",
        "heatmap_title_balls": "Complete frequencies - Balls (1-50)",
        "heatmap_title_stars": "Complete frequencies - Stars (1-12)",
        "heatmap_legend_cold": "Cold (rare)",
        "heatmap_legend_hot": "Hot (frequent)",
    },
    "es": {
        "title": "Informe META DATA EM - 75 Combinaciones",
        "subtitle": "Análisis HYBRIDE EuroMillions",
        "top5_bar": "Top 5 Bolas - Frecuencias",
        "top5_pie": "Distribución Top 5 Bolas",
        "top3_bar": "Top 3 Estrellas - Frecuencias",
        "top3_pie": "Distribución Top 3 Estrellas",
        "frequency": "Frecuencia",
        "balls": "Bolas",
        "stars": "Estrellas",
        "analysis": "Análisis:",
        "no_analysis": "Ningún análisis disponible.",
        "info": "Información:",
        "window_label": "Ventana analizada",
        "engine_label": "Motor",
        "game_label": "Juego: EuroMillions (5 bolas + 2 estrellas)",
        "sponsor_title": "Análisis patrocinado por un Socio Oficial",
        "sponsor_contact": "Colaboraciones y patrocinio:",
        "signature": "Análisis generado por HYBRIDE",
        "engine_desc": "Motor estadístico pedagógico - Módulo EuroMillions",
        "generated_by": "Generado por LotoIA - Módulo META DATA EM",
        "disclaimer1": "Análisis estadístico pedagógico.",
        "disclaimer2": "Ningún resultado está garantizado.",
        "version": "LotoIA — Informe META DATA EM",
        "ai_note": "Este informe ha sido generado íntegramente por inteligencia artificial en colaboración con el motor HYBRIDE.",
        "dev_note": "Gráficos y elementos visuales en desarrollo.",
        "heatmap_title_balls": "Frecuencias completas - Bolas (1-50)",
        "heatmap_title_stars": "Frecuencias completas - Estrellas (1-12)",
        "heatmap_legend_cold": "Frío (raro)",
        "heatmap_legend_hot": "Caliente (frecuente)",
    },
    "pt": {
        "title": "Relatório META DATA EM - 75 Combinações",
        "subtitle": "Análise HYBRIDE EuroMillions",
        "top5_bar": "Top 5 Bolas - Frequências",
        "top5_pie": "Distribuição Top 5 Bolas",
        "top3_bar": "Top 3 Estrelas - Frequências",
        "top3_pie": "Distribuição Top 3 Estrelas",
        "frequency": "Frequência",
        "balls": "Bolas",
        "stars": "Estrelas",
        "analysis": "Análise:",
        "no_analysis": "Nenhuma análise disponível.",
        "info": "Informações:",
        "window_label": "Janela analisada",
        "engine_label": "Motor",
        "game_label": "Jogo: EuroMillions (5 bolas + 2 estrelas)",
        "sponsor_title": "Análise patrocinada por um Parceiro Oficial",
        "sponsor_contact": "Parcerias e patrocínio:",
        "signature": "Análise gerada por HYBRIDE",
        "engine_desc": "Motor estatístico pedagógico - Módulo EuroMillions",
        "generated_by": "Gerado por LotoIA - Módulo META DATA EM",
        "disclaimer1": "Análise estatística pedagógica.",
        "disclaimer2": "Nenhum resultado é garantido.",
        "version": "LotoIA — Relatório META DATA EM",
        "ai_note": "Este relatório foi inteiramente gerado por inteligência artificial em colaboração com o motor HYBRIDE.",
        "dev_note": "Gráficos e elementos visuais em desenvolvimento.",
        "heatmap_title_balls": "Frequências completas - Bolas (1-50)",
        "heatmap_title_stars": "Frequências completas - Estrelas (1-12)",
        "heatmap_legend_cold": "Frio (raro)",
        "heatmap_legend_hot": "Quente (frequente)",
    },
    "de": {
        "title": "META-DATEN EM Bericht - 75 Kombinationen",
        "subtitle": "HYBRIDE EuroMillions-Analyse",
        "top5_bar": "Top 5 Kugeln - Häufigkeiten",
        "top5_pie": "Verteilung Top 5 Kugeln",
        "top3_bar": "Top 3 Sterne - Häufigkeiten",
        "top3_pie": "Verteilung Top 3 Sterne",
        "frequency": "Häufigkeit",
        "balls": "Kugeln",
        "stars": "Sterne",
        "analysis": "Analyse:",
        "no_analysis": "Keine Analyse verfügbar.",
        "info": "Informationen:",
        "window_label": "Analysiertes Fenster",
        "engine_label": "Engine",
        "game_label": "Spiel: EuroMillions (5 Kugeln + 2 Sterne)",
        "sponsor_title": "Analyse gesponsert von einem offiziellen Partner",
        "sponsor_contact": "Partnerschaften & Sponsoring:",
        "signature": "Analyse erstellt von HYBRIDE",
        "engine_desc": "Pädagogische Statistik-Engine - EuroMillions-Modul",
        "generated_by": "Erstellt von LotoIA - META-DATEN EM Modul",
        "disclaimer1": "Pädagogische statistische Analyse.",
        "disclaimer2": "Keine Ergebnisse werden garantiert.",
        "version": "LotoIA — META-DATEN EM Bericht",
        "ai_note": "Dieser Bericht wurde vollständig durch künstliche Intelligenz in Zusammenarbeit mit der HYBRIDE-Engine erstellt.",
        "dev_note": "Diagramme und Grafiken in Entwicklung.",
        "heatmap_title_balls": "Vollständige Häufigkeiten - Kugeln (1-50)",
        "heatmap_title_stars": "Vollständige Häufigkeiten - Sterne (1-12)",
        "heatmap_legend_cold": "Kalt (selten)",
        "heatmap_legend_hot": "Heiss (häufig)",
    },
    "nl": {
        "title": "META DATA EM Rapport - 75 Combinaties",
        "subtitle": "HYBRIDE EuroMillions-analyse",
        "top5_bar": "Top 5 Ballen - Frequenties",
        "top5_pie": "Verdeling Top 5 Ballen",
        "top3_bar": "Top 3 Sterren - Frequenties",
        "top3_pie": "Verdeling Top 3 Sterren",
        "frequency": "Frequentie",
        "balls": "Ballen",
        "stars": "Sterren",
        "analysis": "Analyse:",
        "no_analysis": "Geen analyse beschikbaar.",
        "info": "Informatie:",
        "window_label": "Geanalyseerd venster",
        "engine_label": "Engine",
        "game_label": "Spel: EuroMillions (5 ballen + 2 sterren)",
        "sponsor_title": "Analyse aangeboden door een Offici\u00eble Partner",
        "sponsor_contact": "Partnerschappen & sponsoring:",
        "signature": "Analyse gegenereerd door HYBRIDE",
        "engine_desc": "Educatieve statistische engine - EuroMillions Module",
        "generated_by": "Gegenereerd door LotoIA - META DATA EM Module",
        "disclaimer1": "Educatieve statistische analyse.",
        "disclaimer2": "Geen resultaten worden gegarandeerd.",
        "version": "LotoIA \u2014 META DATA EM Rapport",
        "ai_note": "Dit rapport is volledig gegenereerd door kunstmatige intelligentie in samenwerking met de HYBRIDE-engine.",
        "dev_note": "Grafieken en visualisaties in ontwikkeling.",
        "heatmap_title_balls": "Volledige frequenties - Ballen (1-50)",
        "heatmap_title_stars": "Volledige frequenties - Sterren (1-12)",
        "heatmap_legend_cold": "Koud (zeldzaam)",
        "heatmap_legend_hot": "Heet (frequent)",
    },
}


def _utf8_clean(text):
    """Nettoie le texte : remplace les caracteres Unicode problematiques par des equivalents ASCII safe pour ReportLab/Vera."""
    if not text:
        return ""
    replacements = {
        "\u2192": "->",   # → RIGHTWARDS ARROW
        "\u2190": "<-",   # ← LEFTWARDS ARROW
        "\u2194": "<->",  # ↔ LEFT RIGHT ARROW
        "\u2013": "-",    # – EN DASH
        "\u2014": "-",    # — EM DASH
        "\u2018": "'",    # ' LEFT SINGLE QUOTATION MARK
        "\u2019": "'",    # ' RIGHT SINGLE QUOTATION MARK
        "\u201C": '"',    # " LEFT DOUBLE QUOTATION MARK
        "\u201D": '"',    # " RIGHT DOUBLE QUOTATION MARK
        "\u2026": "...",  # … HORIZONTAL ELLIPSIS
        "\u00A0": " ",    # NO-BREAK SPACE
        "\u2022": "-",    # • BULLET
        "\u25A0": "-",    # ■ BLACK SQUARE
        "\u25A1": "-",    # □ WHITE SQUARE
        "\u2023": "-",    # ‣ TRIANGULAR BULLET
        "\u00B7": ".",    # · MIDDLE DOT
    }
    for char, replacement in replacements.items():
        text = text.replace(char, replacement)
    return text


def _register_fonts(pdfmetrics, TTFont):
    """Enregistre polices UTF-8 : DejaVuSans (Linux/Cloud Run) -> Vera (fallback ReportLab)."""
    import reportlab as _rl
    _rl_fonts = os.path.join(os.path.dirname(_rl.__file__), 'fonts')
    _font_map = {
        'DejaVuSans': [
            os.path.join(_rl_fonts, 'DejaVuSans.ttf'),
            '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf',
            os.path.join(_rl_fonts, 'Vera.ttf'),
        ],
        'DejaVuSans-Bold': [
            os.path.join(_rl_fonts, 'DejaVuSans-Bold.ttf'),
            '/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf',
            os.path.join(_rl_fonts, 'VeraBd.ttf'),
        ],
        'DejaVuSans-Oblique': [
            os.path.join(_rl_fonts, 'DejaVuSans-Oblique.ttf'),
            '/usr/share/fonts/truetype/dejavu/DejaVuSans-Oblique.ttf',
            os.path.join(_rl_fonts, 'VeraIt.ttf'),
        ],
    }
    for _name, _paths in _font_map.items():
        for _path in _paths:
            if os.path.isfile(_path):
                try:
                    pdfmetrics.registerFont(TTFont(_name, _path))
                except Exception:
                    continue
                break


def _validate_graph_data(graph_data):
    """Valide un dict graph_data {labels, values}."""
    return (
        isinstance(graph_data, dict)
        and isinstance(graph_data.get("labels"), list)
        and isinstance(graph_data.get("values"), list)
        and len(graph_data["labels"]) == len(graph_data["values"])
        and len(graph_data["labels"]) > 0
    ) if graph_data else False


def generate_em_graph_image(graph_data_boules: dict, graph_data_etoiles: dict, lang: str = "fr") -> str:
    """
    Genere une image PNG contenant 4 charts :
    - Ligne 1 : Top 5 boules (bar + pie)
    - Ligne 2 : Top 3 etoiles (bar + pie)
    Retourne le chemin du fichier temporaire PNG.
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt

    labels_b = graph_data_boules.get("labels", [])
    values_b = graph_data_boules.get("values", [])
    labels_e = graph_data_etoiles.get("labels", [])
    values_e = graph_data_etoiles.get("values", [])

    bar_colors_b = ['#1A73E8', '#188038', '#F9AB00', '#D93025', '#9334E6']
    pie_colors_b = ['#4285F4', '#34A853', '#FBBC05', '#EA4335', '#A142F4']
    bar_colors_e = ['#FF6D01', '#46BDC6', '#7BAAF7']
    pie_colors_e = ['#E37400', '#009688', '#5C6BC0']

    L = PDF_LABELS.get(lang, PDF_LABELS["fr"])

    fig, axes = plt.subplots(2, 2, figsize=(10, 7))

    # --- Row 1 : Balls ---
    ax1, ax2 = axes[0]
    ax1.bar(labels_b, values_b, color=bar_colors_b[:len(labels_b)])
    ax1.set_title(L["top5_bar"], fontsize=11, fontweight='bold')
    ax1.set_xlabel(L["balls"])
    ax1.set_ylabel(L["frequency"])
    ax1.grid(axis='y', alpha=0.15)

    pie_labels_b = [f"N\u00b0{l} ({v})" for l, v in zip(labels_b, values_b)]
    ax2.pie(
        values_b, labels=pie_labels_b, colors=pie_colors_b[:len(labels_b)],
        autopct='%1.0f%%', startangle=90, counterclock=False,
        textprops={'fontsize': 8},
        wedgeprops={'linewidth': 0.5, 'edgecolor': 'white'}
    )
    ax2.set_title(L["top5_pie"], fontsize=11, fontweight='bold')

    # --- Row 2 : Stars ---
    ax3, ax4 = axes[1]
    ax3.bar(labels_e, values_e, color=bar_colors_e[:len(labels_e)])
    ax3.set_title(L["top3_bar"], fontsize=11, fontweight='bold')
    ax3.set_xlabel(L["stars"])
    ax3.set_ylabel(L["frequency"])
    ax3.grid(axis='y', alpha=0.15)

    pie_labels_e = [f"\u2605{l} ({v})" for l, v in zip(labels_e, values_e)]
    ax4.pie(
        values_e, labels=pie_labels_e, colors=pie_colors_e[:len(labels_e)],
        autopct='%1.0f%%', startangle=90, counterclock=False,
        textprops={'fontsize': 8},
        wedgeprops={'linewidth': 0.5, 'edgecolor': 'white'}
    )
    ax4.set_title(L["top3_pie"], fontsize=11, fontweight='bold')

    plt.tight_layout()

    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    tmp_path = tmp.name
    tmp.close()

    fig.savefig(tmp_path, format="png", dpi=200, facecolor='white', bbox_inches='tight')
    plt.close(fig)

    logger.info(f"[META-PDF-EM] Graph image generee : {tmp_path}")
    return tmp_path


def generate_em_meta_pdf(analysis: str = "", window: str = "75 tirages",
                         engine: str = "HYBRIDE", graph: str = None,
                         graph_data_boules: dict = None,
                         graph_data_etoiles: dict = None,
                         sponsor: str = None,
                         lang: str = "fr",
                         all_freq_boules: dict = None,
                         all_freq_secondary: dict = None) -> io.BytesIO:
    """
    Genere le PDF officiel META75 EuroMillions via ReportLab.
    Affiche DEUX graphiques : boules et etoiles.
    Retourne un BytesIO contenant le PDF.
    """
    from reportlab.pdfgen import canvas
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.units import mm
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont

    logger.info("[META-PDF-EM] Debut generation PDF EuroMillions")

    _register_fonts(pdfmetrics, TTFont)

    L = PDF_LABELS.get(lang, PDF_LABELS["fr"])

    buf = io.BytesIO()
    w, h = A4
    c = canvas.Canvas(buf, pagesize=A4)
    margin_bottom = 30 * mm

    y = h - 40 * mm

    # Titre
    c.setFont("DejaVuSans-Bold", 22)
    c.drawCentredString(w / 2, y, L["title"])
    y -= 12 * mm

    # Sous-titre
    c.setFont("DejaVuSans-Bold", 16)
    c.drawCentredString(w / 2, y, L["subtitle"])
    y -= 10 * mm

    # Separateur
    c.setStrokeColorRGB(0, 0, 0)
    c.setLineWidth(0.5)
    c.line(15 * mm, y, w - 15 * mm, y)
    y -= 10 * mm

    # --- Validation graph_data ---
    valid_boules = _validate_graph_data(graph_data_boules)
    valid_etoiles = _validate_graph_data(graph_data_etoiles)

    logger.info(f"[META-PDF-EM] graph_data_boules valid: {valid_boules}, "
                f"graph_data_etoiles valid: {valid_etoiles}")

    # --- Generation image matplotlib si graph_data valides ---
    generated_graph_path = None
    if valid_boules and valid_etoiles:
        try:
            generated_graph_path = generate_em_graph_image(graph_data_boules, graph_data_etoiles, lang=lang)
            logger.info(f"[META-PDF-EM] PNG genere OK: {generated_graph_path}")
        except Exception as gen_err:
            logger.warning(f"[META-PDF-EM] Generation graph echouee: {gen_err}")

    try:
        # --- Insertion image ---
        effective_graph = generated_graph_path or (graph if graph and os.path.isfile(graph) else "")
        if effective_graph and os.path.isfile(effective_graph):
            try:
                from reportlab.lib.utils import ImageReader
                img = ImageReader(effective_graph)

                img_x = 40
                img_draw_y = y - 260
                img_draw_w = 510
                img_draw_h = 250

                c.drawImage(
                    img, img_x, img_draw_y,
                    width=img_draw_w, height=img_draw_h,
                    preserveAspectRatio=True, mask='auto'
                )
                y = img_draw_y - 8 * mm
                logger.info(f"[META-PDF-EM] Image inseree OK a y={img_draw_y}")
            except Exception as img_err:
                logger.warning(f"[META-PDF-EM] Image ignoree: {img_err}")
        else:
            logger.info("[META-PDF-EM] Aucune image a inserer")

        # Saut de page si espace insuffisant
        if y < margin_bottom:
            c.showPage()
            y = h - 30 * mm

        # Bloc analyse
        c.setFont("DejaVuSans-Bold", 13)
        c.drawString(15 * mm, y, L["analysis"])
        y -= 7 * mm

        analysis_text = _utf8_clean(analysis) or L["no_analysis"]
        text_obj = c.beginText(15 * mm, y)
        text_obj.setFont("DejaVuSans", 11)
        text_obj.setLeading(16)
        max_chars = 90
        for raw_line in analysis_text.split('\n'):
            line = raw_line
            while len(line) > max_chars:
                cut = line[:max_chars].rfind(' ')
                if cut <= 0:
                    cut = max_chars
                text_obj.textLine(line[:cut])
                line = line[cut:].lstrip()
            text_obj.textLine(line)
        c.drawText(text_obj)
        y = text_obj.getY() - 8 * mm

        # Saut de page si espace insuffisant
        if y < margin_bottom:
            c.showPage()
            y = h - 30 * mm

        # Bloc infos
        c.setFont("DejaVuSans-Bold", 13)
        c.drawString(15 * mm, y, L["info"])
        y -= 7 * mm

        c.setFont("DejaVuSans", 11)
        window_text = _utf8_clean(window) or "75 tirages"
        engine_text = _utf8_clean(engine) or "HYBRIDE"
        c.drawString(15 * mm, y, f"{L['window_label']} : {window_text}")
        y -= 6 * mm
        c.drawString(15 * mm, y, f"{L['engine_label']} : {engine_text}")
        y -= 6 * mm
        c.drawString(15 * mm, y, L["game_label"])
        y -= 10 * mm

        # Bloc sponsor si present
        sponsor_text = _utf8_clean(sponsor)
        if sponsor_text:
            c.setStrokeColorRGB(0.7, 0.7, 0.7)
            c.setLineWidth(0.4)
            c.line(15 * mm, y, w - 15 * mm, y)
            y -= 8 * mm

            c.setFillColorRGB(0.1, 0.1, 0.1)
            c.setFont("DejaVuSans", 10)
            c.drawCentredString(w / 2, y, L["sponsor_title"])
            y -= 6 * mm

            c.setFont("DejaVuSans", 9)
            c.drawCentredString(w / 2, y, L["sponsor_contact"])
            y -= 5 * mm

            email_text = "contact@lotoia.fr"
            email_width = c.stringWidth(email_text, "DejaVuSans", 9)
            email_x1 = (w - email_width) / 2
            email_x2 = email_x1 + email_width
            c.drawCentredString(w / 2, y, email_text)
            c.linkURL("mailto:contact@lotoia.fr", (email_x1, y - 2, email_x2, y + 10), relative=0)
            y -= 10 * mm

        # ============ PAGE 2 — HEATMAP (or footer on page 1) ============
        if all_freq_boules:
            from services.pdf_heatmap import draw_heatmap_grid, draw_legend_bar

            # Convert string keys to int
            freq_b = {int(k): v for k, v in all_freq_boules.items()}
            freq_s = {int(k): v for k, v in all_freq_secondary.items()} if all_freq_secondary else {}

            c.showPage()
            y = h - 55

            # Title boules
            c.setFillColorRGB(0, 0, 0)
            c.setFont("DejaVuSans-Bold", 13)
            c.drawCentredString(w / 2, y, L["heatmap_title_balls"])
            y -= 28

            # Grid boules 10x5 = 50 cells, cell 48x28pt, centred
            grid_w = 10 * 48
            grid_x = (w - grid_w) / 2
            y = draw_heatmap_grid(c, grid_x, y, freq_b, range(1, 51), cols=10, cell_w=48, cell_h=28)
            y -= 20

            # Title stars
            c.setFillColorRGB(0, 0, 0)
            c.setFont("DejaVuSans-Bold", 13)
            c.drawCentredString(w / 2, y, L["heatmap_title_stars"])
            y -= 28

            # Grid etoiles 6x2 = 12 cells, cell 48x28pt, centred
            grid_w_s = 6 * 48
            grid_x_s = (w - grid_w_s) / 2
            y = draw_heatmap_grid(c, grid_x_s, y, freq_s, range(1, 13), cols=6, cell_w=48, cell_h=28)
            y -= 25

            # Legend bar
            legend_w = 300
            legend_x = (w - legend_w) / 2
            y = draw_legend_bar(c, legend_x, y, legend_w, 12, L["heatmap_legend_cold"], L["heatmap_legend_hot"])
            y -= 15

            # Separator
            c.setStrokeColorRGB(0, 0, 0)
            c.setLineWidth(0.5)
            c.line(15 * mm, y, w - 15 * mm, y)
            y -= 10 * mm

            # Footer (same as page 1)
            c.setFillColorRGB(0, 0, 0)
            c.setFont("DejaVuSans-Bold", 11)
            c.drawCentredString(w / 2, y, L["signature"])
            y -= 6 * mm
            c.setFont("DejaVuSans", 10)
            c.drawCentredString(w / 2, y, L["engine_desc"])
            y -= 8 * mm
            c.setFont("DejaVuSans-Oblique", 10)
            c.drawCentredString(w / 2, y, L["generated_by"])
            y -= 10 * mm
            c.setFont("DejaVuSans", 9)
            c.drawCentredString(w / 2, y, L["disclaimer1"])
            y -= 5 * mm
            c.drawCentredString(w / 2, y, L["disclaimer2"])
            y -= 10 * mm
            c.setFillColorRGB(150 / 255, 150 / 255, 150 / 255)
            c.setFont("DejaVuSans-Oblique", 8)
            c.drawCentredString(w / 2, y, f"{L['version']} v{APP_VERSION}")

        else:
            # No heatmap data — footer on page 1 (retro-compatibility)
            if y < margin_bottom:
                c.showPage()
                y = h - 30 * mm

            c.setStrokeColorRGB(0, 0, 0)
            c.setLineWidth(0.5)
            c.line(15 * mm, y, w - 15 * mm, y)
            y -= 10 * mm

            c.setFillColorRGB(0, 0, 0)
            c.setFont("DejaVuSans-Bold", 11)
            c.drawCentredString(w / 2, y, L["signature"])
            y -= 6 * mm
            c.setFont("DejaVuSans", 10)
            c.drawCentredString(w / 2, y, L["engine_desc"])
            y -= 8 * mm
            c.setFont("DejaVuSans-Oblique", 10)
            c.drawCentredString(w / 2, y, L["generated_by"])
            y -= 10 * mm
            c.setFont("DejaVuSans", 9)
            c.drawCentredString(w / 2, y, L["disclaimer1"])
            y -= 5 * mm
            c.drawCentredString(w / 2, y, L["disclaimer2"])
            y -= 10 * mm
            c.setFillColorRGB(150 / 255, 150 / 255, 150 / 255)
            c.setFont("DejaVuSans-Oblique", 8)
            c.drawCentredString(w / 2, y, f"{L['version']} v{APP_VERSION}")
            y -= 7 * mm
            c.drawCentredString(w / 2, y, L["ai_note"])
            y -= 7 * mm
            c.drawCentredString(w / 2, y, L["dev_note"])

        c.save()
        buf.seek(0)
        logger.info("[META-PDF-EM] PDF genere OK")

        return buf

    finally:
        if generated_graph_path:
            try:
                os.unlink(generated_graph_path)
            except OSError:
                pass
