...
async def _prepare_chat_context(message: str, history: list, page: str, http_client, lang: str = "fr"):
    ...
    if _skip_sql:
        logger.info(f'[HYBRIDE CHAT] Skip Phase SQL — no data signal | question="{message[:60]}"')
...
def _log_from_meta(meta, module, lang, message, response_preview="",
    ...
    """Helper: call log_chat_exchange from _chat_meta dict."""
    if not meta:
        return
    log_chat_exchange(
        module=module, lang=lang, question=message,
...
async def handle_chat_stream(message: str, history: list, page: str, http_client, lang: str = "fr"):
    ...
    mode = ctx["mode"]
    _stream_chunks = []  # collect for response_preview
