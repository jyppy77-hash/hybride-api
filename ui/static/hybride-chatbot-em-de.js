/* ==============================================================
   HYBRIDE Chatbot Widget — EuroMillions DE (vanilla, IIFE, ES5+)
   Auto-init on #hybride-chatbot-root.
   No dependency. No global pollution.
   Endpoint : /api/euromillions/hybride-chat (lang=de)
   Storage  : hybride-history-em-de
   ============================================================== */
...
(function () {
    ...
    onReady(function () {
        ...
        bubble.setAttribute('aria-label', LI.chatbot_bubble_label || 'HYBRIDE EuroMillions Chatbot \u00f6ffnen');
        ...
        win.innerHTML =
            '<div class="hybride-header">' +
                ...
                '<div class="hybride-header-actions">' +
                    '<button class="hybride-header-clear" aria-label="' + (LI.chatbot_clear_title || 'Neues Gespr\u00e4ch') + '" title="' + (LI.chatbot_clear_title || 'Neues Gespr\u00e4ch') + '">\uD83D\uDDD1\uFE0F</button>' +
                    '<button class="hybride-header-close" aria-label="Close">\u2715</button>' +
                '</div>' +
            '</div>' +
            '<div class="hybride-messages"></div>' +
            '<div class="hybride-input-area">' +
                '<input class="hybride-input" type="text" placeholder="' + (LI.chatbot_placeholder || 'Stelle deine EuroMillions-Frage...') + '" autocomplete="off">' +
        ...
        function showTyping() {
            ...
            el.id = 'hybride-typing-indicator-em-de';
            el.innerHTML =
        ...
        function removeTyping() {
            var el = document.getElementById('hybride-typing-indicator-em-de');
        ...
        function toggle() {
            ...
            if (isOpen) {
                ...
                sponsorViews = 0;
                trackEvent('hybride_em_de_chat_open', {
                    page: detectPage(),
                    has_history: chatHistory.length > 1
                });
                if (window.LotoIA_track) LotoIA_track('chatbot-open', {module: 'euromillions-de'});
            } else {
                win.classList.remove('visible');
                bubble.classList.remove('open');
                root.classList.remove('hybride-fullscreen');
                if (window.LotoIA_track) LotoIA_track('chatbot-close', {module: 'euromillions-de'});
                trackEvent('hybride_em_de_chat_session', {
                    page: detectPage(),
        ...
        /* ==================================
           Page detection (DE URLs)
           ================================== */

        function detectPage() {
            var path = window.location.pathname;
            if (path.indexOf('/de/euromillions/generator') !== -1) return 'generator-em-de';
            if (path.indexOf('/de/euromillions/simulator') !== -1) return 'simulator-em-de';
            if (path.indexOf('/de/euromillions/statistiken') !== -1) return 'statistics-em-de';
            if (path.indexOf('/de/euromillions/verlauf') !== -1) return 'history-em-de';
            if (path.indexOf('/de/euromillions/faq') !== -1) return 'faq-em-de';
            if (path.indexOf('/de/euromillions/nachrichten') !== -1) return 'news-em-de';
            return 'home-em-de';
        }

        var chatHistory = [];
        var WELCOME_TEXT = LI.chatbot_welcome || 'Willkommen! Ich bin HYBRIDE, der KI-Assistent von LotoIA \u2014 EuroMillions-Modul.';
        var STORAGE_KEY = 'hybride-history-em-de';
        ...
        /* ==================================
           Send message (API EuroMillions, lang=de)
           ================================== */
        ...
        function send() {
            ...
            showTyping();
            trackEvent('hybride_em_de_chat_message', {
                page: detectPage(),
                message_length: text.length,
                message_count: messageCount
            });
            if (window.LotoIA_track) LotoIA_track('chatbot-message', {module: 'euromillions-de'});
            ...
            .then(function (data) {
                removeTyping();
                var botText = data.response || (LI.chatbot_error_empty || '\uD83E\uDD16 Antwort nicht verf\u00fcgbar.');
                ...
                if (sponsorId) {
                    sponsorViews++;
                    trackEvent('hybride_em_de_chat_sponsor_view', {
                        page: detectPage(),
            ...
            .catch(function () {
                clearTimeout(timeoutId);
                removeTyping();
                addMessage(LI.chatbot_error_connection || '\uD83E\uDD16 Verbindung unterbrochen. Versuche es in wenigen Sekunden erneut!', 'bot');
                showContactLink();
                trackEvent('hybride_em_de_chat_error', { page: detectPage() });
            });
        }
        ...
        function showContactLink() {
            ...
            a.href = '#';
            a.textContent = LI.contact_chatbot_link || 'Ein Problem? Kontaktiere uns \u2192';
        ...
        var RATING_SOURCE = 'chatbot_em_de';
        ...
        function showRatingWidget() {
            ...
            widget.id = 'hybride-rating-widget-em-de';

            var question = document.createElement('div');
            question.className = 'rating-question';
            question.textContent = LI.chatbot_rating_question || 'Gef\u00e4llt dir HYBRIDE EuroMillions? Bewerte deine Erfahrung!';
            widget.appendChild(question);

            var starsDiv = document.createElement('div');
            starsDiv.className = 'rating-stars';
            starsDiv.id = 'hybride-rating-stars-em-de';
            ...
            for (var i = 1; i <= 5; i++) {
                ...
                star.textContent = '\u2605';
                (function (val) {
                    star.addEventListener('mouseover', function () { highlightStarsEmDe(val); });
                    star.addEventListener('mouseout', function () { resetStarsEmDe(); });
                    star.addEventListener('click', function () { submitChatRatingEmDe(val); });
                })(i);
            ...
            labelBof.textContent = LI.chatbot_rating_low || 'Naja';
            var labelTop = document.createElement('span');
            labelTop.textContent = LI.chatbot_rating_high || 'Klasse!';
            ...
            feedback.id = 'hybride-rating-feedback-em-de';
        ...
        function highlightStarsEmDe(n) {
            var stars = document.querySelectorAll('#hybride-rating-stars-em-de .rating-star');
        ...
        function resetStarsEmDe() {
            var selected = document.querySelector('#hybride-rating-stars-em-de .rating-star.selected');
            if (selected) return;
            var stars = document.querySelectorAll('#hybride-rating-stars-em-de .rating-star');
            for (var i = 0; i < stars.length; i++) { stars[i].classList.remove('active'); }
        }

        function submitChatRatingEmDe(rating) {
            // V141 A.4 UX Fix 1 — 3 tiers : low (1-2) Pflicht 20 Zeichen, mid (3) / high (4-5) optional
            var stars = document.querySelectorAll('#hybride-rating-stars-em-de .rating-star');
            for (var i = 0; i < stars.length; i++) {
                if (i < rating) {
            ...
            showRatingCommentSectionEmDe(rating);
        }

        function showRatingCommentSectionEmDe(rating) {
            var widget = document.getElementById('hybride-rating-widget-em-de');
            if (!widget) return;
            ...
            if (isMandatory) {
                ...
                dismissBtn.setAttribute('aria-label', LI.rating_close || 'Schließen');
                dismissBtn.textContent = '✕';
                dismissBtn.addEventListener('click', function () { dismissRatingCommentEmDe(); });
            ...
            if (tier === 'low') {
                prompt.textContent = LI.chatbot_rating_prompt_low || 'Was hat nicht funktioniert?';
            } else if (tier === 'mid') {
                prompt.textContent = LI.chatbot_rating_prompt_mid || 'Vorschlag zur Verbesserung? (optional)';
            } else {
                prompt.textContent = LI.chatbot_rating_prompt_high || 'Ein Wort, das uns freut? (optional) 🤩';
            }
            ...
            if (isMandatory) {
                ...
                errorMsg.textContent = LI.chatbot_rating_min_chars || 'Mindestens 20 Zeichen bitte 😊';
            ...
            if (!isMandatory) {
                ...
                skipBtn.textContent = LI.chatbot_rating_skip || 'Überspringen';
                skipBtn.addEventListener('click', function () {
                    if (submitted) return;
                    submitted = true;
                    skipBtn.disabled = true;
                    _sendRatingPayloadEmDe(rating, '');
                });
            ...
            submitBtn.textContent = LI.chatbot_rating_send || 'Senden';
            ...
            submitBtn.addEventListener('click', function () {
                ...
                _sendRatingPayloadEmDe(rating, comment.substring(0, 500));
            });
        ...
        function dismissRatingCommentEmDe() {
            ...
            if (window.LotoIA_track) LotoIA_track('rating-dismissed-popup', {module: 'euromillions-de'});
        }

        function _sendRatingPayloadEmDe(rating, comment) {
            ...
            .then(function (data) {
                if (data.success) {
                    ...
                    var feedback = document.getElementById('hybride-rating-feedback-em-de');
                    var messages = {
                        5: LI.chatbot_rating_5 || 'Danke! Du bist klasse!',
                        4: LI.chatbot_rating_4 || 'Danke! Freut uns, dass es dir gef\u00e4llt!',
                        3: LI.chatbot_rating_3 || 'Danke! Wir werden uns verbessern!',
                        2: LI.chatbot_rating_2 || 'Danke f\u00fcr dein Feedback!',
                        1: LI.chatbot_rating_1 || 'Danke! Sag uns, was wir verbessern k\u00f6nnen!'
                    };
                    if (feedback) {
                        feedback.textContent = messages[rating] || (LI.chatbot_rating_default || 'Danke!');
                        feedback.style.display = 'block';
                    }
                    setTimeout(function () {
                        var w = document.getElementById('hybride-rating-widget-em-de');
                        if (w) w.innerHTML = '<div class="rating-thanks">' + (LI.chatbot_rating_done || 'Danke f\u00fcr dein Feedback!') + '</div>';
                    }, 3000);
                }
            })
            .catch(function (err) { /* silent */ });

            trackEvent('hybride_em_de_chat_rating', {
                page: detectPage(),
                rating: rating,
                message_count: messageCount,
                has_comment: hasComment
            });
            if (window.LotoIA_track) LotoIA_track('rating-submitted', {rating: rating, module: 'euromillions-de', has_comment: hasComment});
        }
        ...
        function clearConversation() {
            trackEvent('hybride_em_de_chat_clear', {
                page: detectPage(),
                message_count: messageCount
            });
            chatHistory = [];
