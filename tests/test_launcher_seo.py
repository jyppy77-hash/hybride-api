...
class TestVersion:
    ...
    def test_app_version(self):
        ...
        # News transparence + lexique bump : 1.6.045 → 1.6.046 (10/06/2026 — article news Loto FR « LotoIA renforce sa transparence » en featured + JSON-LD NewsArticle, PDF lexique copié ui/static/lexique-lotoia-fr.pdf servi /ui/static/, encadré jeu responsable aligné footer exact, volet EM reporté post-i18n, zéro changement Python runtime)
        # Lot SEO générateur bump : 1.6.046 → 1.6.047 (10/06/2026 — audit SEO findings #8+#9 : désambiguïsation /loto Simulateur→Générateur title+H1+og:title+twitter:title+JSON-LD+mini-FAQ « loto IA » préservé ; /loto/exploration 200+canonical → 301 /loto dans main.py, route+mapping retirés de pages.py, ligne Allow robots.txt retirée, _SEO_ROUTES nettoyé ; /loto/analyse intact)
        # Lot SEO #5 lastmod sitemap bump : 1.6.047 → 1.6.048 (11/06/2026 — finding #5 audit SEO on-page : <lastmod> par catégorie dans routes/sitemap.py — dynamiques = date dernier tirage Loto/EM en base (réutilise _fetch_last_draw_date_loto/_em), news = LAST_NEWS_DATE (config/version.py), éditoriales + launcher = _EDITORIAL_LASTMOD dates git harmonisées, fallback LAST_DEPLOY_DATE si DB down ; sitemap toujours 200 ; structure URLs/hreflang/priorités inchangée)
        # V143 bump : 1.6.048 → 1.6.049 (11/06/2026 — retry 429 chat streaming pattern V129.1 opt-in max_retries cap 12s + garde anti-double-émission SSE _yielded_any + timeout 1er-token 15s distinct inter-chunk 8s V131.F + error_detail différencié Vertex429/InterChunkTimeout/VertexError/NoChunks via failure_box + corps exception loggé 5 handlers 429 ; cause : NoChunks 52%/24h = 429 DSQ régional Vertex gemini-2.5-flash europe-west1 incompressible, audits READ-ONLY 11/06)
