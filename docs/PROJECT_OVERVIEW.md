...
│   ├── chat_pipeline_em.py              # HYBRIDE chatbot orchestration — EM (771L, Phase 4+P9 SSE+Phase A+Sponsor V3)
...
│   ├── chat_utils.py                    # Formatting, context, sponsor — Loto+EM (433L, Phase 1 + Sponsor V3)
...
### Admin Back-Office — routes/admin.py (Phase 2-4/4 + RT-1)
...
| `/admin/api/impressions` | GET | JSON: impressions KPI + chart + table (filterable by period, event, lang, device) |
...
| HYBRIDE Chatbot (EuroMillions) | Stable | Modular: 4 service modules (Phase 4). 13-phase detection (Phase A: argent, 6 langs). SSE streaming (P9). Same architecture as Loto. 7 FR + 7 EN pages (Phase 11). Isolated sessionStorage. EN response pools + EN prompts. Sponsor V3: `module="em"`, dynamic EM codes. |
