...
hybride-api/
│
├── main.py                              # FastAPI orchestrator (~722L, 12 middlewares, 21 routers)
...
│   ├── api_data_unified.py              # Unified data endpoints /api/{game}/... (Phase 10, ~670L, +pairs)
...
│   ├── api_sponsor_track.py             # Sponsor event tracking — POST /api/sponsor/track (Phase 1/4, E5 banner documented)
│   ├── api_track.py                     # Universal event tracking — POST /api/track (Phase RT-1, ~100L)
│   └── admin.py                         # Admin back-office — dashboard, impressions (sponsor_id+tarif filter), votes, engagement, realtime (period+by_type+CSV/PDF export), sponsors CRUD, factures CRUD, tarifs EU (EI/SASU switch), config (~1446L, Phase 2-4/4 + RT-1 + Tarifs + Engagement + RT-2)
│
...
│   ├── chat_utils.py                    # Formatting, context, sponsor — Loto+EM (501L, Phase 1 + Sponsor V3 + Pairs + Triplets + Phase G)
│   ├── chat_utils_em.py                 # Formatting, context — EM (277L, Phase 4 + Pairs + Triplets + Phase G)
...
│   ├── em_pdf_generator.py              # ReportLab PDF — EM (649L, dual graphs + heatmap page 2, i18n 6 langs, sponsor V3 dynamic)
...
│   └── admin_pdf.py                     # Admin PDF generation — sponsor reports + invoices + realtime reports (360L, FacturIA palette, Phase 3+4/4 + RT-2)
│
...
│   ├── pt/LC_MESSAGES/messages.po/.mo   # Portuguese catalog (complete, 749 entries, Sprint PT)
│   ├── es/LC_MESSAGES/messages.po/.mo   # Spanish catalog (complete, 749 entries, Sprint ES)
│   ├── de/LC_MESSAGES/messages.po/.mo   # German catalog (complete, 749 entries, Sprint DE)
│   └── nl/LC_MESSAGES/messages.po/.mo   # Dutch catalog (complete, 749 entries, Sprint NL)
│
├── ui/                                  # Frontend layer
...
│   │
│   ├── templates/admin/                 # Jinja2 templates — Admin back-office (Glassmorphism UI, Phase 2-4/4 + RT-1 + Tarifs + Engagement)
│   │   ├── _base.html                  # Admin base layout (sticky glass nav, theme toggle, Inter font, 5 nav links)
│   │   ├── login.html                  # Admin login page (glass card, gradient buttons)
│   │   ├── dashboard.html              # Dashboard KPI (glass cards, nav links)
│   │   ├── impressions.html            # Impressions detail (AJAX filters, sponsor_id dropdown, by_sponsor summary, Chart.js, pagination)
│   │   ├── votes.html                  # Votes detail (distribution, filters, pagination)
│   │   ├── engagement.html             # Engagement dashboard (chatbot/rating/simulateur/meta75 — KPI, category breakdown, stacked bar chart, paginated table)
│   │   ├── realtime.html               # Realtime feed (period toggle, by-type KPI cards, activity heatmap, CSV/PDF export, LIVE pulse)
...
│       ├── admin.js                     # Admin AJAX logic (impressions, votes, realtime (period toggle, by-type KPI cards, export), chart.js, 6/6 sponsor events)
...
│       ├── Sponsors_media/              # Sponsor media assets (14 product code folders: LOTO_FR_A/B + 12 EM_XX_A/B)
...
│   └── sponsors.json                    # Sponsor system config V3 (14 product codes: LOTO_FR + 12 EM, A/B slots, 7 slot groups)
│
...
- **Pair correlations**: Top co-occurring ball pairs (C(5,2)=10 pairs/draw via `UNION ALL` SQL). Star pair correlations for EM (simple `GROUP BY`). Cached 1h. API: `GET /api/{game}/stats/pairs?top_n=10&window=5A`
- **Triplet correlations**: Top co-occurring ball triplets (C(5,3)=10 triplets/draw via `UNION ALL` SQL). Canonical ordering via `LEAST/GREATEST/SUM-MIN-MAX`. Cached 1h. API: `GET /api/{game}/stats/triplets?top_n=10&window=5A`
...
| **Sponsor system** | Post-Gemini injection via `_get_sponsor_if_due(history, lang, module)`. 14 product codes (LOTO_FR_A/B + 12 EM_XX_A/B). Rotation A/B: msg 3→A, msg 6→B, msg 9→A... `[SPONSOR:ID]` marker parsed by frontend. Config in `config/sponsors.json` V3 (7 slot groups). 6 tracking events. Does not pollute conversational history. |
...
| **Backend** | `api_chat_em.py` (thin wrapper) → `services/chat_pipeline_em.py`: 14-phase detection pipeline adapted for EuroMillions (boules 1-50, 2 étoiles 1-12, table tirages_euromillions, draw days mardi/vendredi). Phase G: grid generation via HYBRIDE engine. |
...
| **i18n (Phase 11)** | EN support via `lang` parameter: EN response pools (`chat_responses_em_en.py`), EN chatbot prompts (`prompt_hybride_em_en.txt`), EN sponsor text (`_get_sponsor_if_due(history, lang="en", module="em")`). Frontend widget: `hybride-chatbot-em-en.js` with `extractSponsorId()` regex `[SPONSOR:xx]`. |
| **Sponsor V3** | Dynamic EM product codes: `_get_sponsor_if_due(..., module="em")` reads `em_{lang}` slots from sponsors.json. Frontend JS derives `EM_{LANG}_A/B` via `LI.locale`. 6 emplacements (E1-E6) all tracked with `sponsor_id`. |
...
| **Phase P — Pair Correlations** | Regex `_detect_paires()` (6 langs) | `get_pair_correlations()`, `get_star_pair_correlations()` (EM) | "quelles paires sortent ensemble", "quel numéro sort avec le 15", "which pairs come together" |
...
**Detection priority**: Insult → Compliment → **Argent** → Continuation → Next Draw → Draw Results → [Temporal filter check] → Grid → Complex → **Pairs** → Single Number → Text-to-SQL
...
**Temporal filter bypass**: If the message contains a temporal filter ("en 2025", "depuis janvier 2026", "cette année", etc.), Phases 2/3/P/OOR/1 are skipped via `_has_temporal_filter()` (73 regex patterns, 6 langs) and Phase SQL handles the query directly. **Critical**: if Phase SQL fails (NO_SQL), the pipeline does NOT fall back to Phase 3 (`get_classement_numeros()`) which has no date filter — this prevents returning misleading all-time stats when the user asked for a specific period.
...
**Context enrichment**: Stats are injected as tagged blocks (`[DONNÉES TEMPS RÉEL]`, `[RÉSULTAT TIRAGE]`, `[RÉSULTAT SQL]`, `[ANALYSE DE GRILLE]`, `[CLASSEMENT]`, `[COMPARAISON]`, `[NUMÉROS CHAUDS/FROIDS]`, `[CORRÉLATIONS DE PAIRES]`) into the Gemini user message. The system prompt contains rules to use these facts without making predictions.
...
|  routes/api_sponsor_track.py  Sponsor tracking    |
|  routes/api_track.py     Universal event tracking  |
|  routes/admin.py         Admin back-office (7 pages)|
|  main.py                 health, SEO 301          |
+--------------------------------------------------+
         |
         v
+--------------------------------------------------+
|              SERVICES LAYER (23 modules, ~8073L)  |
|                                                   |
|  ── Chat Pipeline (Phase 1+4+A+G) ──────────     |
|  chat_pipeline.py       Loto 14-phase orchestration|
|  chat_pipeline_em.py    EM 14-phase orchestration  |
...
services/ — 23 modules, ~8717 lines
    ...
    ├── chat_utils.py        (501L)  Context building, formatting, sponsor, generation, triplets — Loto
    ├── chat_utils_em.py     (277L)  Context building, formatting, generation, triplets — EuroMillions
    ├── chat_responses_em_en.py (277L)  EN response pools for EM chatbot (Phase 11+Phase A)
    ── Stats Layer (Phase 2: base class refactoring + Pairs + Triplets) ──
    ├── base_stats.py       (1070L)  GameConfig-driven base class (11 methods, 4 SQL hooks, pairs+triplets)
    ├── stats_service.py     (123L)  Loto thin wrapper → base_stats
    ├── em_stats_service.py   (90L)  EM thin wrapper → base_stats (em: cache prefix)
    ── AI, Cache, PDF ──
...
tests/ — 1437 tests, 34 files (pytest + pytest-cov)
    ...
    ── Pairs/Triplets Correlation Tests (Phase P) ──
    ├── test_pairs.py             (464L)   32 tests: get_pair_correlations, star pairs, detection 6 langs, formatting, API endpoint
    ├── test_triplets.py          (389L)   22 tests: get_triplet_correlations, detection 6 langs, formatting, API endpoint
...
routes/api_data_unified.py    (~693L)  13 data endpoints — /api/{game}/tirages/*, stats/*, draw/*, triplets, etc.
...
| `admin.js` | Admin dashboard JS: impressions (sponsor+tarif filter, by_sponsor breakdown), votes, realtime (period toggle, by-type KPI cards, export URLs), engagement (KPI, category breakdown, stacked bar chart, pagination) |
...
| `/api/{game}/stats/pairs` | GET | Top co-occurring ball pairs (top_n, window). EM: includes star_pairs |
| `/api/{game}/stats/triplets` | GET | Top co-occurring ball triplets (top_n, window) |
...
### Sponsor Tracking — routes/api_sponsor_track.py (Phase 1/4)

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/sponsor/track` | POST | Track sponsor events (6 types: popup-shown, click, video-played, inline-shown, result-shown, pdf-downloaded). `sponsor_id` required (LOTO_FR_X / EM_XX_X). Owner IP filter, SHA-256 session hash, rate limit 10/min, 204 response |

### Universal Event Tracking — routes/api_track.py (Phase RT-1)

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/track` | POST | Track all frontend events. Owner IP filter, SHA-256 session hash, rate limit 30/min, 204 response. Payload: `{event, page, module, lang, device, meta}` |

### Admin Back-Office — routes/admin.py (Phase 2-4/4 + RT-1 + Engagement)

Auth: cookie-based (`lotoia_admin_token`), timing-safe comparison (`secrets.compare_digest`).

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/admin/login` | GET/POST | Login page + password auth |
| `/admin/logout` | GET | Clear auth cookie |
| `/admin` | GET | Dashboard (KPI: impressions, votes, avg rating) |
| `/admin/impressions` | GET | Impressions detail page (AJAX filters) |
| `/admin/votes` | GET | Votes detail page (AJAX filters) |
| `/admin/realtime` | GET | Realtime event feed page (5s polling, period toggle, by-type KPI cards) |
| `/admin/api/impressions` | GET | JSON: impressions KPI + chart + table + by_sponsor breakdown (filterable by period, event, lang, device, sponsor_id) |
| `/admin/api/votes` | GET | JSON: votes summary + distribution + table (filterable by period, source, rating) |
| `/admin/engagement` | GET | Engagement detail page (non-sponsor events: chatbot, rating, simulateur, meta75) |
| `/admin/api/engagement` | GET | JSON: engagement KPI + by_category + chart + table (filterable by period, event, module, lang, device) |
| `/admin/api/realtime` | GET | JSON: last 100 events + KPI (total/hour/types) + by_type counts + event type list. Params: `event_type`, `period=today\|week\|month` |
| `/admin/export/realtime/csv` | GET | CSV export of realtime events (up to 5000 rows). Params: `event_type`, `period` |
| `/admin/export/realtime/pdf` | GET | PDF export of realtime report (FacturIA style: KPI + by_type + detail). Params: `event_type`, `period` |
| `/admin/sponsors` | GET | Sponsors list |
| `/admin/sponsors/new` | GET/POST | Create sponsor |
| `/admin/sponsors/{id}` | GET/POST | Edit sponsor |
| `/admin/factures` | GET | Invoices list |
| `/admin/factures/new` | GET/POST | Create invoice (auto-calculates from sponsor_impressions × grille tarifaire) |
...
| `/admin/export/votes/csv` | GET | CSV export of votes |
...
- **11 config-driven methods**: `get_frequency()`, `get_retard()`, `get_classement()`, `get_top_n()`, `get_numeros_par_categorie()`, `get_comparaison_numeros()`, `analyze_grille_for_chat()`, `prepare_grilles_pitch_context()`, `get_pair_correlations()`, `get_triplet_correlations()`, `get_star_pair_correlations()`
- **4 SQL hooks**: Overridable for game-specific column queries (UNION ALL patterns)
- **Pair correlations**: `get_pair_correlations()` computes C(5,2)=10 pairs per draw via 10 `UNION ALL` + `LEAST/GREATEST` + `GROUP BY`. `get_star_pair_correlations()` for EM étoiles (simple `GROUP BY`, 1 pair/draw). Both cached 1h with `{prefix}pairs:{window}:{top_n}` key.
- **Triplet correlations**: `get_triplet_correlations()` computes C(5,3)=10 triplets per draw via 10 `UNION ALL`. Canonical ordering: `LEAST(a,b,c)` for num_a, `a+b+c-LEAST-GREATEST` for num_b, `GREATEST(a,b,c)` for num_c. Cached 1h with `{prefix}triplets:{window}:{top_n}` key.
...
**services/chat_utils.py** (501L) / **chat_utils_em.py** (277L):
- **Context building**: `_build_session_context()`, `_enrich_with_context()` (continuation)
- **Formatting**: `_format_tirage_context()`, `_format_stats_context()`, `_format_grille_context()`, `_format_triplets_context()`
- **Sponsor**: `_get_sponsor_if_due(history, lang, module="loto"|"em")` — rotation A/B (msg 3→A, 6→B, 9→A...), `[SPONSOR:ID]` marker, 14 product codes (LOTO_FR_A/B + 12 EM_XX_A/B), config from `sponsors.json` V3
...
### services/admin_pdf.py — Admin PDF Generation (Phase 3+4/4 + RT-2)

- **Palette**: FacturIA style (BLEU_FONCE=#1a2744, DORE=#d4a843, GRIS=#8891a4)
- **`generate_sponsor_report_pdf(data)`**: Sponsor impressions report with KPI summary + detail table
- **`generate_invoice_pdf(facture, config)`**: Professional invoice with emitter/client side-by-side, line items, TVA calculation, TTC total in bleu fonce background
- **`generate_realtime_report_pdf(kpi, by_type, table_data, period_label)`**: Realtime events report — KPI header (total/hour/types) + by_type breakdown table + event detail table (up to 200 rows)
- **Layout**: Header with gold accent, side-by-side info blocks, zebra-striped tables, right-aligned totals
- **Used by**: `routes/admin.py` (export PDF, invoice PDF download, realtime PDF export)
...
| `ADMIN_TOKEN` | Yes (admin) | — | Admin back-office auth cookie token (timing-safe comparison) |
| `ADMIN_PASSWORD` | Yes (admin) | — | Admin login password |
| `OWNER_IP` | Optional | — | Owner IPv4 address(es) for GA4 + Umami + Wysistat + tracker.js analytics filtering (pipe-separated) |
| `OWNER_IPV6` | Optional | — | Owner IPv6 prefix for GA4 + Umami + Wysistat + tracker.js analytics filtering |
...
| Owner IP Filtering | UmamiOwnerFilterMiddleware injects `window.__OWNER__=true` for owner IPs → blocks Umami (via `umamiBeforeSend`), GA4 (via `bootGtagImmediately` early return), Wysistat (via `isOwner` bridge), and tracker.js (via `window.__OWNER__` check) |
| Admin Auth | Cookie-based auth (`lotoia_admin_token`) with `secrets.compare_digest()` timing-safe comparison. Password login with SHA-256. `noindex, nofollow` on all admin pages |
| Sponsor/Event Tracking | SHA-256 session hash (IP+UA+date), no raw IP stored (RGPD). Owner IP filtered silently (204 response) |
...
| `test_chat_temporal_bypass.py` | 9 | Temporal bypass regression: force_sql + SQL fail → no Phase 3 fallback (Loto + EM pipelines) |
...
| `translations/{fr,en,pt,es,de,nl}/` | 6-language `.po` catalogs with compiled `.mo` files. All 6 complete (749 entries each). FR=reference. |
...
### Admin Phase 1/4 — Sponsor Tracking (2026-03-04)

| Change | Detail |
|--------|--------|
| `routes/api_sponsor_track.py` | POST /api/sponsor/track — owner IP filter, SHA-256 session hash, rate limit 10/min |
| `sponsor_impressions` table | event_type, page, lang, country, device, session_hash, user_agent_hash |
| `ui/static/sponsor-popup.js` + EM variants | Fire-and-forget POST to `/api/sponsor/track` on popup-shown, click, video-played |

### Admin Phase 2/4 — Dashboard + Impressions + Votes (2026-03-05)

| Change | Detail |
|--------|--------|
| `routes/admin.py` | Login/logout, dashboard KPI, impressions/votes pages + JSON API endpoints |
| `ui/templates/admin/` | 6 templates: _base.html, login.html, dashboard.html, impressions.html, votes.html |
| `ui/static/admin.js` | AJAX filters, Chart.js bar charts, table sorting, pagination |
| `ui/static/admin.css` | Dark theme, FacturIA palette |
| `tests/test_admin.py` | 32 tests (auth, dashboard, impressions, votes API) |

### Admin Phase 3+4/4 — Exports + FacturIA (2026-03-05)

| Change | Detail |
|--------|--------|
| `routes/admin.py` | +15 routes: CSV/PDF exports, sponsors CRUD, factures CRUD, config |
| `services/admin_pdf.py` | PDF generation (FacturIA palette: BLEU_FONCE=#1a2744, DORE=#d4a843) — sponsor reports + invoices + realtime reports |
| `migrations/003_create_facturia_tables.sql` | 4 tables: fia_config_entreprise, fia_sponsors, fia_grille_tarifaire, fia_factures |
| 6 new admin templates | sponsors.html, sponsor_form.html, factures.html, facture_form.html, facture_detail.html, config.html |
| `tests/test_admin.py` | +28 tests (exports, sponsors CRUD, factures CRUD, config) → 60 total |

### Admin Styling + Theme Toggle (2026-03-05)

| Change | Detail |
|--------|--------|
| `ui/static/admin.css` | Complete UI overhaul: 40+ CSS custom properties, FacturIA palette (bleu fonce #1a237e + dore #d4a843), button hierarchy, zebra-striped tables, badge statuses |
| Light/dark theme | `body.light` class override, cookie persistence (`admin_theme`, 1 year TTL), flash prevention (inline script reads cookie before render) |
| All admin templates | Inline styles → CSS classes, consistent responsive design |

### Phase RT-1 — Universal Event Tracking + Realtime Feed (2026-03-05)

| Change | Detail |
|--------|--------|
| `migrations/004_create_event_log.sql` | `event_log` table (event_type, page, module, lang, device, country, session_hash, meta_json, created_at) with indexes |
| `routes/api_track.py` | POST /api/track — owner IP filter, SHA-256 session hash, rate limit 30/min, 204 response |
| `ui/static/tracker.js` | `LotoIA_track(event, meta)` — dedup (2s TTL), sendBeacon, owner filter, auto-detect page/lang/device |
| 34 HTML files + EM _base.html | Added `<script src="/ui/static/tracker.js?v=1" defer>` after analytics.js |
| 10 JS files (36 insertions) | Added `LotoIA_track()` alongside every `umami.track()` call (chatbot, rating, sponsor, simulateur, meta75) |
| `main.py` | Import + include `track_router` (21 routers total) |
| `ui/templates/admin/realtime.html` | Live feed page: period toggle (today/7d/30d), by-type KPI cards, activity heatmap, CSV+PDF export, auto-refresh 5s |
| `ui/static/admin.js` | `initRealtime()` — polling `/admin/api/realtime?period=`, by-type card rendering, period toggle, export URL sync |
| `ui/static/admin.css` | Realtime styles: `.rt-type-card` glassmorphism, `.rt-period-btn` toggle, `.rt-dot` pulse, `.rt-badge-*` colored per event type |
| `routes/admin.py` | +4 endpoints: GET /admin/realtime (page) + GET /admin/api/realtime (JSON, period+by_type) + GET /admin/export/realtime/csv + GET /admin/export/realtime/pdf |
| `ui/templates/admin/_base.html` | "Realtime" nav link with green dot indicator |
| `tests/test_track.py` | 12 tests (valid, invalid, rate limit, owner filter, device sanitization, session hash, country detection, meta JSON) |

### Sponsor System V3 — Product Codes + EM Replication (2026-03-06)

5-step sponsor refactoring: remove hardcoded sponsor names, introduce 14 product codes, replicate to EuroMillions.

| Step | Change | Detail |
|------|--------|--------|
| 1/5 | Product codes | Remove EmovisIA hardcodes, add LOTO_FR_A (Premium) / LOTO_FR_B (Standard), i18n sponsor labels (6 langs) |
| 2/5 | Chatbot rotation | `sponsors.json` V2 with `slots.loto_fr.slot_a/slot_b`. Rotation A/B in `_get_sponsor_if_due()`: msg 3→A, 6→B, 9→A... `[SPONSOR:ID]` marker parsed by frontend |
| 3/5 | Unified placements | E1-E6 all use `sponsor_id` in POST `/api/sponsor/track`. E5 result banner added. 6 tracking events |
| 4a/5 | EM backend | `sponsors.json` V3: 12 EM slots (6 langs x A/B). `module="em"` param in `_get_sponsor_if_due()`. `chat_pipeline_em.py` + `em_pdf_generator.py` adapted |
| 4b/5 | EM frontend | 5 JS files updated: dynamic `EM_{LANG}_A/B` via `LI.locale`. `extractSponsorId()` regex in both chatbot files. `injectSponsorBannerEM()` in simulateur-em.js. 14 `Sponsors_media/` folders |
| Admin fix | `_VALID_EVENTS` | `routes/admin.py`: 3→6 events (aligned with tracking endpoint) |

**14 product codes**: LOTO_FR_A/B, EM_FR_A/B, EM_EN_A/B, EM_ES_A/B, EM_PT_A/B, EM_DE_A/B, EM_NL_A/B

**6 tracking events**: sponsor-popup-shown, sponsor-click, sponsor-video-played, sponsor-inline-shown, sponsor-result-shown, sponsor-pdf-downloaded

**6 emplacements**: E1 (popup), E2 (video META 75), E3 (PDF mention), E4 (chatbot inline), E5 (result banner), E6 (PDF download tracking)

**Files modified** (20+): `config/sponsors.json`, `services/chat_utils.py`, `services/chat_pipeline_em.py`, `services/em_pdf_generator.py`, `services/pdf_generator.py`, `routes/api_sponsor_track.py`, `routes/admin.py`, `ui/static/sponsor-popup.js`, `ui/static/sponsor-popup75.js`, `ui/static/sponsor-popup75-em.js`, `ui/static/em/sponsor-popup-em.js`, `ui/static/hybride-chatbot.js`, `ui/static/hybride-chatbot-em.js`, `ui/static/hybride-chatbot-em-en.js`, `ui/static/simulateur.js`, `ui/static/simulateur-em.js`, `config/js_i18n.py`, `tests/test_chat_utils.py`, `tests/test_sponsor_track.py`
...
| `tests/test_admin.py` | +5 realtime tests (RT-1) + 13 RT-2 tests (period, by_type, CSV, PDF) → 135 admin total |
...
| Admin back-office | None | **Full admin**: dashboard KPI, impressions/votes detail, realtime feed, sponsors CRUD, invoices CRUD, config, CSV/PDF exports. Dark/light theme. FacturIA palette |
| Event tracking | Umami only (external) | **Triple+internal**: Umami + GA4 + Wysistat + **event_log** (36 LotoIA_track calls, tracker.js on all pages) |
| Realtime feed | None | **Live event feed** (5s polling, period toggle today/7d/30d, by-type KPI cards, activity heatmap, CSV+PDF export) |
| Tests | 248 | **1217** |
| Sponsor system | Basic (config/sponsors.json, 3 events) | **V3**: 14 product codes, 7 slot groups, 6 emplacements, 6 events, A/B rotation, `[SPONSOR:ID]` marker, dynamic EM lang |
| Services modules | 10 | **23** |
...
| 2026-03-03 | Phase A: Argent/money/gambling detection — 13th pipeline phase. Court-circuit (no Gemini). 3-level escalation (L1 pedagogical, L2 firm, L3 help redirection). 6-lang response pools + word lists (FR/EN/ES/PT/DE/NL). Country-specific gambling support links. Phase G exclusion guard added 2026-03-08. **972 tests.** |
...
| 2026-03-05 | **Admin Phase 1/4**: Sponsor tracking — POST /api/sponsor/track (owner IP filter, SHA-256 hash, rate limit). `sponsor_impressions` table. |
| 2026-03-05 | **Admin Phase 2/4**: Dashboard + impressions + votes pages. Cookie auth, AJAX filters, Chart.js, pagination. 6 Jinja2 templates. 32 tests. |
...
| 2026-03-08 | **Sponsor Audit Phase 1**: Impressions dashboard `sponsor_id` filter — dropdown (14 product codes), `by_sponsor` breakdown table+chart, sponsor column in detail table, CSV export. 8 new tests. |
| 2026-03-08 | **Engagement Dashboard (Phase 2)**: New `/admin/engagement` page for non-sponsor events. 10 event types in 4 categories (chatbot/rating/simulateur/meta75). Filters: period, event, module, lang, device. KPI bar (6 metrics), category breakdown table, stacked bar Chart.js chart, paginated detail table. `_build_engagement_where()` with `NOT LIKE 'sponsor-%%'` guard. Nav link added. 14 new tests. |
...
| Area | Status | Notes |
|------|--------|-------|
| Backend (FastAPI + aiomysql) | Stable | ~722L orchestrator, 21 routers, native async DB (Phase 5), 12 middlewares |
...
| Admin Back-Office | **Stable (Phase 2-4/4 + RT-1 + RT-2 + Tarifs + Engagement)** | Premium glassmorphism UI. Dashboard KPI, impressions detail (sponsor_id+tarif filter + by_sponsor breakdown), votes detail, **engagement dashboard** (chatbot/rating/simulateur/meta75 — KPI, category breakdown, stacked bar chart, pagination), **realtime feed** (period toggle today/7d/30d, by-type KPI cards with live counters, activity heatmap, CSV+PDF export, LIVE pulse), sponsors CRUD, invoices CRUD+PDF, tarifs EU (14 codes, EI/SASU switch, packs, paliers), config. Dark/light theme. CSV/PDF exports. Cookie auth (timing-safe). 135 admin tests |
...
| Event Tracking | **Stable (Phase RT-1 + Cleanup)** | Universal `event_log` table + POST /api/track. `tracker.js` (LotoIA_track) on all pages (35 HTML + EM base template). 36 tracked events (chatbot, rating, sponsor, simulateur, meta75). Owner IP filter, SHA-256 session hash, RGPD compliant. Dual-tracking cleanup: 10 duplicate `LotoIA_track()` calls removed from 4 sponsor-popup JS files (sponsor events now tracked exclusively via `/api/sponsor/track`). 12 track tests |
...
| Tests & Quality | 6.5 | **9.4** | +2.9 | 248 → **1437 tests** (+480%), 34 test files, sitemap 100% coverage, chat detectors (multilang top-N, temporal filter, Phase T neutralizer, Phase G generation, triplets 6 langs), temporal bypass regression tests, pairs+triplets correlations, i18n, prompts, multilang routes, hreflang, PDF heatmap, legal pages, SEO headers+schemas, admin back-office (135 tests: realtime period+export, impressions tarif filter), event tracking (12 tests) all tested |
