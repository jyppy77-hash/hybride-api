...
Toggle via EM_PUBLIC_ACCESS env var on Cloud Run (default: true since launch 15/03/2026).

Access hierarchy:
  1. EM_PUBLIC_ACCESS=True → everyone passes
  2. Owner IP (/64 match) → direct access
  3. Everyone else → 302 redirect
...
async def em_access_middleware(request: Request, call_next):
    ...
    Access hierarchy: public toggle → owner IP → block.
    """
