...
-- Complexité : 🟡
-- Data loss potentielle : oui (admin_config contient valeurs saisies manuellement + tarifs modifiés post-seed)
-- PITR requis : OBLIGATOIRE
-- ============================================================
-- -- ⚠️ ATTENTION : admin_config contient des valeurs modifiées via admin UI post-seed
-- -- (billing_mode, ei_siret, sasu_siret, TVA, IBAN, etc. — V117 "config entreprise fully populated").
-- -- sponsor_tarifs peut aussi avoir été modifié par la migration 006 et/ou via /admin/tarifs.
-- --
-- -- ⚠️ PRÉ-REQUIS : si la migration 006 a été appliquée, son rollback est implicite avec ce drop.
-- --
-- -- ÉTAPE 1 : exporter IMPÉRATIVEMENT les 2 tables avant drop
-- SELECT * FROM admin_config;
-- SELECT * FROM sponsor_tarifs;
-- -- Exporter les 2 tables (voir migrations/ROLLBACK_PROCEDURE.md §Export)
-- -- Sans export, admin UI post-rollback = config entreprise vide → PDFs factures cassés.
--
-- -- ÉTAPE 2 : drop dans l'ordre (aucune FK entre les 2, ordre arbitraire)
-- DROP TABLE IF EXISTS sponsor_tarifs;
-- DROP TABLE IF EXISTS admin_config;
-- -- Note : /admin/config + /admin/tarifs + services/admin_pdf.py (facture header) inopérants.
-- -- Dashboard KPI billing_mode fallback à "EI" hardcoded. Re-seed + re-saisie admin UI obligatoire.
